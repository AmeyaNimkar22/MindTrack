    ...
    <nav className="flex items-center justify-between px-8 py-4 bg-white shadow-md relative z-50">
      ...
      {isOpen && (
        <div className="absolute top-16 left-0 w-full bg-white border-t border-gray-200 shadow-md flex flex-col items-center space-y-4 py-4 md:hidden">
          <Link to="/" className="hover:text-blue-600" onClick={toggleMenu}>Home</Link>
          <Link to="/about" className="hover:text-blue-600" onClick={toggleMenu}>Know more</Link>
          <Link to="/resources" className="hover:text-blue-600" onClick={toggleMenu}>Resources</Link>
          <Link to="/test" className="hover:text-blue-600" onClick={toggleMenu}>Test</Link>
        </div>
      )}
    </nav>
  );
